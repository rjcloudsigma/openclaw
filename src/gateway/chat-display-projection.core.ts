...
import { isTimeoutErrorMessage } from "../agents/embedded-agent-helpers/errors.js";
...
const GATEWAY_ASSISTANT_TIMEOUT_FALLBACK_TEXT =
  "The selected model timed out before producing a reply. Retry the request; if it recurs, choose another model or clear the session model selection with /model default.";
...
function isTimeoutAssistantError(message: Record<string, unknown>): boolean {
  return [message.errorCode, message.errorType, message.errorMessage].some(
    (value) => typeof value === "string" && isTimeoutErrorMessage(value),
  );
}

function getAssistantErrorFallbackText(message: Record<string, unknown>): string {
  if (isContextOverflowAssistantError(message)) {
    return GATEWAY_ASSISTANT_CONTEXT_OVERFLOW_FALLBACK_TEXT;
  }
  if (isTimeoutAssistantError(message)) {
    return GATEWAY_ASSISTANT_TIMEOUT_FALLBACK_TEXT;
  }
  return GATEWAY_ASSISTANT_ERROR_FALLBACK_TEXT;
}
